...
export async function refreshCandidateAnalysis(runtime: AnalysisRuntime, ticker: string) {
  ...
  const candidate = await runtime.DB.prepare("SELECT ticker FROM buy_candidates WHERE ticker=? AND active=1").bind(symbol).first<{ ticker: string }>();
  ...
  return {
    ...
    dca_multiplier: decision.multiplier, action: decision.action,
